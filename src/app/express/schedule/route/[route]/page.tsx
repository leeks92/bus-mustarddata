...
import { getExpressRoute, getExpressRoutes, getExpressTerminals, formatCharge, getValidMinCharge, getValidMaxCharge } from '@/lib/data';
...
// 노선 슬러그 파싱 (romanized slug: seoulgyeongbu-busan 형태)
function parseRouteSlugFromParam(slug: string): { depSlug: string; arrSlug: string } | null {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    const depId = getExpressTerminalIdBySlug(depSlug);
    const arrId = getExpressTerminalIdBySlug(arrSlug);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${route.depTerminalName}에서 ${route.arrTerminalName} 가는 고속버스 시간표. ${route.schedules.length}회 운행${chargeText}.`,
    keywords: [
      `${depName} ${arrName} 버스`,
    ...
    alternates: {
      canonical: `${BASE_URL}/express/schedule/route/${routeSlug}`,
    },
    openGraph: {
      title: `${depName} → ${arrName} 고속버스 시간표`,
      description: `${route.depTerminalName}에서 ${route.arrTerminalName} 가는 고속버스. ${route.schedules.length}회/일 운행${chargeText}.`,
      url: `${BASE_URL}/express/schedule/route/${routeSlug}`,
      type: 'website',
    },
    twitter: {
      card: 'summary',
...
export default async function ExpressRoutePage({ params }: Props) {
  ...
  if (!parsed) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-8 text-center">
        ...
        <Link href="/express/schedule" className="text-indigo-600 hover:underline">
          고속버스 터미널 목록으로 돌아가기
        </Link>
      </div>
    );
  ...
  const breadcrumbItems = [
    ...
    { name: route.depTerminalName, url: `${BASE_URL}/express/schedule/${depTerminalSlug}` },
    { name: `${route.depTerminalName} → ${route.arrTerminalName}`, url: `${BASE_URL}/express/schedule/route/${routeSlug}` },
  ];
  ...
  const faqItems = [
    {
      question: `${route.depTerminalName}에서 ${route.arrTerminalName}까지 고속버스 요금은 얼마인가요?`,
      answer: `${route.depTerminalName}에서 ${route.arrTerminalName}까지 고속버스 요금은 ${formatCharge(minCharge)}${minCharge !== maxCharge ? `부터 ${formatCharge(maxCharge)}` : ''}입니다. 버스 등급(${gradeList})에 따라 요금이 다릅니다.`,
    },
    {
    ...
    },
    {
      ...
      answer: `${route.depTerminalName}에서 ${route.arrTerminalName}까지 고속버스 소요시간은 ${estimatedDuration || '노선에 따라 상이합니다'}입니다. 도로 상황에 따라 달라질 수 있습니다.`,
    },
    {
      question: `${route.depTerminalName} ${route.arrTerminalName} 고속버스 하루 몇 회 운행하나요?`,
      answer: `${route.depTerminalName}에서 ${route.arrTerminalName}까지 하루 총 ${schedules.length}회 운행됩니다. ${gradeList} 등급의 버스가 운행되며, 성수기에는 임시 배차가 추가될 수 있습니다.`,
    },
    {
      question: `${route.depTerminalName} ${route.arrTerminalName} 고속버스 예매는 어디서 하나요?`,
      answer: `고속버스통합예매(KOBUS, www.kobus.co.kr)에서 온라인 예매가 가능합니다. 앱으로도 예매 가능하며, 출발 1시간 전까지 무료 취소가 가능합니다.`,
    },
    {
      question: `${route.depTerminalName} ${route.arrTerminalName} 고속버스 어린이·청소년 할인이 되나요?`,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <TableJsonLd
        name={`${route.depTerminalName} → ${route.arrTerminalName} 고속버스 시간표`}
        description={`${route.depTerminalName}에서 ${route.arrTerminalName}까지 고속버스 시간표. ${schedules.length}회 운행, ${formatCharge(minCharge)}부터.`}
      ...
      <nav className="text-sm text-gray-600 mb-6">
        ...
        <Link href="/express/schedule" className="hover:text-indigo-600">
          고속버스 시간표
        </Link>
        <span className="mx-2">›</span>
        <Link href={`/express/schedule/${depTerminalSlug}`} className="hover:text-indigo-600">
      ...
      {/* B3: 교통수단 비교 */}
      <TransportCompareSection depName={route.depTerminalName} arrName={route.arrTerminalName} minCharge={minCharge} duration={estimatedDuration} />
      ...
      <section className="mt-8 p-4 bg-gray-50 rounded-lg">
        ...
        <Link
          href={`/express/schedule/route/${reverseRouteSlug}`}
          ...
          <div>
            ...
            <div className="space-y-2">
              {sameDepRoutes.map((r, idx) => (
                <Link
                  key={idx}
                  href={`/express/schedule/route/${createRouteSlug(r.depTerminalName, r.arrTerminalName)}`}
    ...
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      ...
      {guide.tips && guide.tips.length > 0 && (
        <div className="mt-4 bg-indigo-50 rounded-lg p-4">
          <h3 className="font-bold text-indigo-800 mb-2 text-sm">이용 팁</h3>
          <ul className="space-y-1">
            {guide.tips.map((tip, idx) => (
              <li key={idx} className="text-sm text-indigo-700 flex gap-2">
...
function TransportCompareSection({ depName, arrName, minCharge, duration }: { depName: string; arrName: string; minCharge: number; duration: string | null }) {
  // 주요 노선에 대한 KTX/SRT 비교 데이터
  const ktxRoutes: Record<string, { ktxPrice: string; ktxTime: string; ktxStation: string }> = {
    '서울경부-부산': { ktxPrice: '약 59,800원', ktxTime: '약 2시간 30분', ktxStation: '서울역 → 부산역' },
    '서울경부-동대구': { ktxPrice: '약 43,500원', ktxTime: '약 1시간 40분', ktxStation: '서울역 → 동대구역' },
    '서울경부-대전복합': { ktxPrice: '약 23,700원', ktxTime: '약 50분', ktxStation: '서울역 → 대전역' },
    '서울경부-울산': { ktxPrice: '약 55,300원', ktxTime: '약 2시간 10분', ktxStation: '서울역 → 울산역' },
    '서울호남-광주': { ktxPrice: '약 43,000원', ktxTime: '약 1시간 30분', ktxStation: '용산역 → 광주송정역' },
    '서울호남-전주': { ktxPrice: '약 31,000원', ktxTime: '약 1시간 30분', ktxStation: '용산역 → 전주역' },
    '동서울-강릉': { ktxPrice: '약 27,600원', ktxTime: '약 1시간 50분', ktxStation: '청량리역 → 강릉역' },
    '서울경부-강릉': { ktxPrice: '약 27,600원', ktxTime: '약 1시간 50분', ktxStation: '청량리역 → 강릉역' },
  };

  // 출발-도착에서 터미널 제거하여 키 생성
  const depClean = depName.replace(/터미널$/, '').replace(/종합버스$/, '');
  const arrClean = arrName.replace(/터미널$/, '').replace(/종합버스$/, '');
  const routeKey = `${depClean}-${arrClean}`;
  const ktxData = ktxRoutes[routeKey];

  if (!ktxData) return null;

  return (
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      <h2 className="text-xl font-bold mb-4 text-gray-900">
        <span className="mr-2">&#x1F504;</span> 고속버스 vs KTX 비교
      </h2>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b-2 border-gray-200">
              ...
              <th className="py-3 px-4 text-center font-bold text-indigo-700 bg-indigo-50 rounded-t-lg">고속버스</th>
              <th className="py-3 px-4 text-center font-bold text-gray-700">KTX/SRT</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-gray-100">
              <td className="py-3 px-4 font-medium text-gray-700">요금</td>
              <td className="py-3 px-4 text-center bg-indigo-50 font-bold text-indigo-700">{formatCharge(minCharge)}~</td>
            ...
            <tr className="border-b border-gray-100">
              ...
              <td className="py-3 px-4 text-center bg-indigo-50 text-gray-700">{duration || '-'}</td>
              <td className="py-3 px-4 text-center text-gray-700">{ktxData.ktxTime}</td>
            </tr>
            <tr className="border-b border-gray-100">
              <td className="py-3 px-4 font-medium text-gray-700">탑승 위치</td>
              <td className="py-3 px-4 text-center bg-indigo-50 text-gray-700">{depName}</td>
            ...
            <tr>
              ...
              <td className="py-3 px-4 text-center bg-indigo-50 text-gray-700 text-xs">저렴한 요금, 좌석 선택 다양</td>
              <td className="py-3 px-4 text-center text-gray-700 text-xs">빠른 이동, 정시 운행</td>
            </tr>
          </tbody>
        </table>
      </div>
      <p className="text-xs text-gray-500 mt-3">* KTX 요금은 일반석 정상 요금 기준이며, 할인 및 변동이 있을 수 있습니다.</p>
    </section>
  );
}
    ...
    <section className="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-6">
      ...
      <div className="space-y-3 text-sm text-amber-800">
        <div className="flex gap-3">
          ...
          <p><strong>설날·추석 연휴:</strong> {depName} → {arrName} 노선은 명절 2주 전부터 예매가 시작됩니다. 고속버스통합예매(KOBUS)에서 사전 예매를 하지 않으면 좌석 확보가 어려울 수 있습니다.</p>
        </div>
        <div className="flex gap-3">
          ...
          <p><strong>임시 배차:</strong> 명절 연휴, 여름 휴가철(7~8월), 겨울 스키시즌에는 임시 배차가 추가됩니다. 정확한 시간표는 출발일 기준 예매 사이트에서 확인하세요.</p>
        </div>
        <div className="flex gap-3">
