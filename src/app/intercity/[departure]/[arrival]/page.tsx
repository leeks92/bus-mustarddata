...
import { getIntercityRoute, getIntercityRoutes, formatCharge } from '@/lib/data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const route = getIntercityRoute(departure, arrival);
  ...
  return {
    ...
    description: `${route.depTerminalName}에서 ${route.arrTerminalName} 가는 시외버스 시간표. ${route.schedules.length}회 운행, 요금 ${formatCharge(route.schedules[0]?.charge || 0)}부터.`,
    keywords: [
      `${depName} ${arrName} 버스`,
    ...
    alternates: {
      canonical: `${BASE_URL}/intercity/${departure}/${arrival}`,
    },
    openGraph: {
      ...
      url: `${BASE_URL}/intercity/${departure}/${arrival}`,
      type: 'website',
    },
    twitter: {
      card: 'summary',
...
export default async function IntercityRoutePage({ params }: Props) {
  ...
  const breadcrumbItems = [
    { name: '홈', url: BASE_URL },
    { name: '시외버스', url: `${BASE_URL}/intercity` },
    { name: `${route.depTerminalName} → ${route.arrTerminalName}`, url: `${BASE_URL}/intercity/${departure}/${arrival}` },
  ];
  ...
  const faqItems = [
    {
      ...
      answer: `${route.depTerminalName}에서 ${route.arrTerminalName}까지 시외버스 요금은 ${formatCharge(minCharge)}${minCharge !== maxCharge ? `부터 ${formatCharge(maxCharge)}` : ''}입니다. 버스 등급(일반, 우등, 프리미엄)에 따라 요금이 다릅니다.`,
    },
    {
