...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const terminalId = getIntercityTerminalIdBySlug(decodedSlug);
  ...
  return {
    title: `${terminal.terminalNm} 시외버스 시간표 - 요금, 노선 안내`,
    description: `${terminal.terminalNm}에서 출발하는 시외버스 시간표와 요금 정보. ${routes.length}개 노선 운행.`,
    keywords: [
      `${terminal.terminalNm} 시외버스`,
      `${terminal.terminalNm} 시간표`,
      `${terminal.terminalNm} 버스 요금`,
      '시외버스 예매',
    ],
    alternates: {
      canonical: `${BASE_URL}/intercity/schedule/${decodedSlug}`,
    },
    openGraph: {
      title: `${terminal.terminalNm} 시외버스 시간표`,
      description: `${terminal.terminalNm} 시외버스 시간표와 요금 정보를 확인하세요.`,
      url: `${BASE_URL}/intercity/schedule/${decodedSlug}`,
      type: 'website',
    },
    twitter: {
      card: 'summary',
...
export default async function IntercityTerminalPage({ params }: Props) {
  const { terminal: terminalSlug } = await params;
  const decodedSlug = decodeURIComponent(terminalSlug);
  const terminalId = getIntercityTerminalIdBySlug(decodedSlug);
  const terminal = terminalId ? getTerminal(terminalId) : null;
  ...
  // 도착지별로 정렬
  const sortedRoutes = intercityOnlyRoutes.sort((a, b) =>
  ...
  const breadcrumbItems = [
    ...
    { name: terminal.terminalNm, url: `${BASE_URL}/intercity/schedule/${decodedSlug}` },
  ];
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <header className="bg-gradient-to-r from-slate-600 to-slate-700 text-white rounded-xl p-6 mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-bold mb-2">
          {terminal.terminalNm} 시외버스 시간표
        </h1>
        <p className="opacity-90">{terminal.cityName || '시외버스 터미널'}</p>
              ...
              {terminalInfo.phone && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a href={`tel:${terminalInfo.phone}`} className="text-slate-600 hover:underline font-medium">
                      {terminalInfo.phone}
                    </a>
                  </div>
                </div>
      ...
      {/* 출발 노선 목록 */}
      <section>
        <h2 className="text-xl font-bold mb-4">{terminal.terminalNm}에서 출발하는 시외버스</h2>
        {sortedRoutes.length === 0 ? (
          <div className="bg-gray-50 rounded-lg p-8 text-center">
            <p className="text-gray-500 mb-4">현재 수집된 노선 정보가 없습니다.</p>
            <p className="text-sm text-gray-400">시외버스 시간표는 당일 운행 정보만 조회 가능하여 데이터 수집에 제한이 있습니다.</p>
          </div>
        ) : (
      ...
      <section className="mt-8 text-sm text-gray-600">
        <p>
          {terminal.terminalNm}에서 출발하는 시외버스 시간표입니다. 
          {sortedRoutes.length > 0 
            ? `총 ${sortedRoutes.length}개 노선이 운행됩니다.`
